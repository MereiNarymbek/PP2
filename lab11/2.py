...
#  қолданушыны name және phone арқылы қосу бар болса  update
def insert_or_update_user():
    ...
    try:
        cur.callproc("insert_or_update_user", (name, phone))  # постгрескюл процедурасын шақыру
        conn.commit()
        print("Процедура сәтті орындалды.")
    except Exception as err:  # Тек осы жол өзгертілді
        print(f"ERROR: {err}")
    finally:
        cur.close()
        conn.close()
